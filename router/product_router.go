...
func ProductRouter(rg *gin.RouterGroup, cfg *config.Config, userClient userpb.UserServiceClient, productHandler *handler.ProductHandler) {
	...
	admin := rg.Group("/admin", security.RequireAuth(accessName, secretKey, userClient), security.RequireMultiRoles([]string{common.RoleAdmin}))
	{
		admin.POST("/categories", productHandler.CreateCategory)
		admin.GET("/categories", productHandler.GetAllCategoriesAdmin)
		admin.GET("/categories/:id", productHandler.GetCategoryByID)
		...
		admin.GET("/images", productHandler.GetImagesByProductID)
	}
}
